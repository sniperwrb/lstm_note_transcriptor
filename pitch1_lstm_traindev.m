...
for iter=1:itermax
    ...
    for dataid=1:ltrain
        ...
        load(['mirpv1/',names{perm(dataid)},'.mat']);
        sres=size(allres);
        spv=size(pv);
        % nx=sres(1);
        % ny=spv(1);
    ...
    save(['savs/pitch1_lstm_',num2str(iter),'.mat'],'Wc','Wi','Wf','Wo',...
...
save('pitch1_lstm.mat','Wc','Wi','Wf','Wo','Uc','Ui','Uf','Uo','Why',...
